...
rasch_quality_children <- function(df_nest, vars_metric) {
  ...
  df_nest <- df_nest %>% 
    mutate(itemfit_start = purrr::map(mod_start, ~ msq.itemfit(.)$itemfit),
           ...
           tthresh_start = purrr::map(mod_start, ~ tam.threshold(.)),
           ...
           WLE_start = purrr::map(mod_start, ~ tam.wle(., progress = FALSE)),
  ...
  df_nest <- df_nest %>% 
    mutate(itemfit_multigroup = purrr::map(mod_multigroup, ~ msq.itemfit(.)$itemfit),
           ...
           xsithresh_multigroup = purrr::map(mod_multigroup, ~ cbind(.$xsi[vars_metric[["common"]],],
                                                              tam.threshold(.)[vars_metric[["common"]], ])),
           tthresh_multigroup = purrr::map(mod_multigroup, ~ tam.threshold(.)),
           ...
           PCA_multigroup = purrr::map(cor_multigroup, ~ ifelse(any(is.na(.)),
                                                         "NA in residual correlation matrix for start model. Unable to calculate eigenvalues and eigenvectors. Consider changing your testlets.",
                                                         list(eigen(.)$vectors[,1]))),
           WLE_multigroup = purrr::map(mod_multigroup, ~ tam.wle(., progress = FALSE)),
    ...
    df_nest <- df_nest %>% 
      mutate(itemfit_anchored = purrr::map(mod_anchored, ~ msq.itemfit(.)$itemfit),
             ...
             xsithresh_anchored = purrr::map(mod_anchored, ~ cbind(.$xsi[vars_metric[["common"]],],
                                                            tam.threshold(.)[vars_metric[["common"]], ])),
             tthresh_anchored = purrr::map(mod_anchored, ~ tam.threshold(.)),
             ...
             PCA_anchored = purrr::map(cor_anchored, ~ ifelse(any(is.na(.)),
                                                       "NA in residual correlation matrix for start model. Unable to calculate eigenvalues and eigenvectors. Consider changing your testlets.",
                                                       list(eigen(.)$vectors[,1]))),
             WLE_anchored = purrr::map(mod_anchored, ~ tam.wle(., progress = FALSE)),
